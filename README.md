...
### Introduction
This is my first mini project and one of my milestones before starting to dive deep into data analytics world. My idea was to scrape all the online listing price from a website and put the data into a map, so that I can retrieve the data by only typing the longitude and latitude of a certain location. There are several ways to scrape the website, such as using Selenium, BeautifulSoup, Scrapy, etc., but in my project, I use BeautifulSoup because I thought it was the easiest way to start learning web scraping.

### The Python Code
First thing first, we have to import the required library to start the web scraping process. I use the following libraries to start.
...
- ```requests``` is an HTTP library that allows us to send HTTP/1.1 request easily. This will act like as if the website is opened by human.
  For more information about request, click [here](https://pypi.org/project/requests/).
...
- ```BeautifulSoup``` is a library that makes it easy to scrape information from web pages.
  It sits atop an HTML or XML parser, providing Pythonic idioms for iterating, searching, and modifying the parse tree. For more information about BeautifulSoup click [here](https://pypi.org/project/beautifulsoup4/).
...
- ```time``` is used to delay the scraping process, so that it will not detected as a DDoS attack.
...
- ```header``` is used as a cover, so that the website does not detect the scraping process as a robot
